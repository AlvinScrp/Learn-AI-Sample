...
os.environ["DASHSCOPE_API_KEY"] = "sk-xxxx"  # 替换为实际密钥
...
class TextProcessingTool:
    ...
    def run(self, input_str: str) -> str:
        ...
        参数:
            input_str: JSON字符串，包含operation和content等参数
        返回:
            处理结果
        """
        try:
...
Action Input: 工具的输入
观察: 工具的结果
... (Thought/Action/Action Input/观察可以重复 N 次)
Thought: 我现在已经有了最终答案
回答: 对原始问题的最终回答
...
Thought: {agent_scratchpad}"""
    ...
    with warnings.catch_warnings():
        ...
        agent_executor = AgentExecutor.from_agent_and_tools(
            ...
            verbose=True,
            handle_parsing_errors=True  # 启用错误处理和自动重试
        )
